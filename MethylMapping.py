...
def methylMap450(methylation_beta_array):
...
def averageGene(mapped_sample_data, gene):
    """Averages mapped methylation beta array beta values from specified gene (not scientific)"""

    df = mapped_sample_data.groupby(by=mapped_sample_data.columns, axis=1).mean()
    df = pd.DataFrame(df[gene])
    print(df.head())
